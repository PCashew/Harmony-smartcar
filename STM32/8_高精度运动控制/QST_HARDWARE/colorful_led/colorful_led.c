...
u8 L_ws_data[ws_num];
u8 R_ws_data[ws_num];
static u8 g_carLightState = 0xFF;
/**************************************************************************
...
static void CarLight_ClearBuffers(void)
{
    u8 i;
    for (i = 1; i <= led_num; ++i) {
        L_ws2812_rgb(i, WS_DARK);
...
static void CarLight_Refresh(void)
{
    __disable_irq();
    L_ws2812_refresh(led_num);
    R_ws2812_refresh(led_num);
...
void CarLight_Forward(void)
{
    u8 i;
    if (g_carLightState == 1) return;
    g_carLightState = 1;
    CarLight_ClearBuffers();
    for (i = 1; i <= led_num; ++i) {
        L_ws2812_rgb(i, 80, 80, 80);
    }
    CarLight_Refresh();
}

void CarLight_Backward(void)
{
    u8 i;
    if (g_carLightState == 2) return;
    g_carLightState = 2;
    CarLight_ClearBuffers();
    for (i = 1; i <= led_num; ++i) {
        R_ws2812_rgb(i, 100, 0, 0);
    }
    CarLight_Refresh();
}

void CarLight_TurnLeft(void)
{
    u8 i;
    if (g_carLightState == 3) return;
    g_carLightState = 3;
    CarLight_ClearBuffers();
    for (i = 1; i <= 3; ++i) {
        L_ws2812_rgb(i, 100, 55, 0);
        R_ws2812_rgb(i, 100, 55, 0);
    }
    CarLight_Refresh();
}

void CarLight_TurnRight(void)
{
    u8 i;
    if (g_carLightState == 4) return;
    g_carLightState = 4;
    CarLight_ClearBuffers();
    for (i = 4; i <= 6; ++i) {
        L_ws2812_rgb(i, 100, 55, 0);
...
void CarLight_Stop(void)
{
    u8 i;
    if (g_carLightState == 0) return;
    g_carLightState = 0;
    CarLight_ClearBuffers();
    for (i = 1; i <= led_num; ++i) {
        L_ws2812_rgb(i, 70, 0, 0);
        R_ws2812_rgb(i, 70, 0, 0);
    }
    CarLight_Refresh();
}
